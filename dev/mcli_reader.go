...
func main() {
	...
	for {
		...
		//_, err = conn.Write([]byte("mcli-debug-mode on;trace pt sink stderr; trace pt level developer; trace pt verbosity complete; trace pt start now\n"))
		//data, err := io.ReadAll(conn)
		//if err != nil {
		//	fmt.Fprintf(os.Stderr, "Failed to read all data: %v\n", err)
		//	return
		//}

		//fmt.Printf(">>> Done: Read %d bytes \n", len(data))
		//fmt.Printf("--- Full Message Received ---\n%s\n", data)

		//ime.Sleep(10 * time.Second)

		loop:
		for i := 1; i <= 3; i++ {
			_, err = conn.Write([]byte("@1 show stat\n"))
			if err != nil {
