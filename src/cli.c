...
static int _send_status(char **args, char *payload, struct appctx *appctx, void *private)
{
	...
	BUG_ON((strcmp(args[1], "READY") != 0),
		"Triggered in _send_status, expect to receive only status PROC_O_READY.\n");
	BUG_ON((!args[2]),
		"Triggered in _send_status, expect to receive worker PID.\n");
	...
	list_for_each_entry(proc, &proc_list, list) {
		if ((proc->options & PROC_O_TYPE_WORKER) &&
			(proc->options & PROC_O_LEAVING) &&
			(proc->reloads >= 1) && (proc->sigterm_sent == 0)) {
