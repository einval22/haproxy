...
/* Sets current and max nofile limits for the process. It may terminate the
...
/* Sets current and max memory limits for the process. It may terminate the
...
/* Checks current process nofile limit via getrlimit and the possibility to
 * preallocate FD if global.prealloc_fd is set. It may terminate the process, if
 * its current nofile limit is lower than global.maxsock
 * (approx. global.maxconn*2) and there is no 'no strict-limits' in the global
 * section.
 */
void check_current_nofile_limit(void)
{
	struct rlimit limit;
