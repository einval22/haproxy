 ...
 *
 * Copyright 2024 Willy Tarreau <w@1wt.eu>
 *
 ...
 */
int compute_ideal_maxconn(struct rlimit *nofile_lim_at_boot, struct proxy *proxies_list)
{
	int ssl_sides = !!global.ssl_used_frontend + !!global.ssl_used_backend;
	int engine_fds = global.ssl_used_async_engines * ssl_sides;
	int pipes = compute_ideal_maxpipes(proxies_list);
	int remain = MAX(nofile_lim_at_boot->rlim_cur, nofile_lim_at_boot->rlim_max);
	int maxconn;
