...
int readcfgfile(const struct cfgfile *file)
{
	...
	if ((thisline = calloc((sizeof(*thisline) * linesize), sizeof(*thisline))) == NULL) {
...
next_line:
	...
	while(fgets_from_mem(thisline + readbytes, linesize - readbytes, &cur_position, file->content + file->size)) {
