			  ...
			  struct flt_conf *fconf, char **err, void *private)
{
	...
	if (ret == -1) {
		memprintf(err, "Could not open configuration file %s : %s",
			  cfg_file.filename, strerror(errno));
		goto error;
	}
	if (ret & (ERR_ABORT|ERR_FATAL)) {
		memprintf(err, "Error(s) found in configuration file %s", cfg_file.filename);
		goto error;
	}

	/* Check SPOE agent */
	if (curagent == NULL) {
		memprintf(err, "No SPOE agent found in file %s", cfg_file.filename);
		goto error;
	}
